...
const filterByAll = (setData, filter) => {
  ...
  onValue(queryFilter, (snapshot) => {
    ...
    if (filter.keyword != "All" && filter.keyword != null) {
    ...
    if (filter.tag != "All" && filter.tag != null) {
      data = data.filter((item) => {
        if (item.tag != null) {
    ...
    if (filter.sizes != "All") {
      data = data.filter((item) =>
        item.sizes.includes(filter.sizes.toLowerCase())
      );
    }
...
const orderByPrice = (setData, byOrder = "increase", data) => {
  ...
  if (byOrder == "increase") listData.sort((a, b) => a.price - b.price);
  else if ((byOrder = "decrease")) listData.sort((a, b) => b.price - a.price);
...
export { filterByAll, orderByPopular, orderByPrice, orderByRating };
